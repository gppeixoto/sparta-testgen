...
public class Main {
  ...
  enum OPCODE {BIPUSH, ISTORE, ILOAD, ICONST, 
    ...
    FMUL, FNEG, FSTORE, FLOAD, LSTORE, LSTORE_0, LSTORE_1, LSTORE_2, LSTORE_3,
  };
  ...
  public void replay() {
    ...
    for(int i = 0; i < instructionTrace.size(); i++) {
      ...
      case LDC:
        ...
        try {
          ...
          operandStack.push(k);
            ...
            try {
              double f = Double.parseDouble(complementOne);
              operandStack.push(f);
      ...
      case FMUL:
        float f1 = ((Double) operandStack.pop()).floatValue();
        float f2 = ((Double) operandStack.pop()).floatValue();
        float r = f1 * f2;    
        operandStack.push(r);           
        break;
      ...
      case LSTORE_0:
        operandStack.store(0);
        break;

      case LSTORE_1:
	    operandStack.store(1);
        break;

      case LSTORE_2:
        operandStack.store(2);
        break;

      case LSTORE_3:
        operandStack.store(3);
        break;
