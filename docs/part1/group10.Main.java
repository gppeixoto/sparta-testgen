...
public class Main {
  ...
  enum OPCODE {BIPUSH, ISTORE, ILOAD, ICONST, 
    ...
    FREM, FRETURN, FSTORE, FSUB, LSUB, LUSHR, NOP, POP2, LSTORE, FLOAD, LLOAD
  ...
  public void replay() {
    ...
    for(int i = 0; i < instructionTrace.size(); i++) {
      ...
      case FREM: 
	float op1 = (Float) ((Double) operandStack.pop()).floatValue();
	float op2 = (Float) ((Double) operandStack.pop()).floatValue();
	float rep = (op1 % op2); 
	operandStack.push(new Double(rep)); 
	break;

      case FRETURN: 
	float value = (Float) ((Double) operandStack.pop()).floatValue();
	operandStack = callStack.pop(); 
	...
	break;
      ...
      case FSUB:
        ...
        float resp = f1 - f2;
        operandStack.push(resp);     
        break;

      case LSUB:
        long l1 = ((Integer) operandStack.pop()).longValue(); 
        long l2 = ((Integer) operandStack.pop()).longValue();
        long respl = l1 - l2;
	...
	long lo2 = ((Integer) operandStack.pop()).longValue();
	long resu = lo2 >>> lo1; 
